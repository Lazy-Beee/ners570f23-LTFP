...
def write_to_csv(time_list, position_list, layer_info, case_name):
    data1 = {}
    ...
    for layer_count in range(num_layers) :    # Layer loop
        ...
        if (layer_count % 2) == 0 :    # Scan from X- to X+
            for i in range(layer_scan_count_X):
                ...
                pos = layer_pos_X[i]
                pos[0][1] += layer_count * increment_thickness
                pos[1][1] += layer_count * increment_thickness
...
if __name__ == '__main__':
    ...
    write_to_csv(time_list, position_list, layer_info, case_name)
